<%
'=====================================================================================
' Document :  EasyIDE Library
' Version  :  v2.2
' Author   :  Alan
' Contact  :  www.FreeContex.com
' Update   :  2010-10
'=====================================================================================
...
const OBJ_DICT  = "Scripting.Dictionary"
...
'函数：将ASP代码段运行结果返回为字串
function ob_get_contents(str)
	dim tmp, a, b, t, matches, m
	tmp = "dim htm, htm_t : htm_t = htm : htm = """""&vbcrlf
	a = 1
	b = instr(a,str,"<%")+2
	while b > a+1
		t = mid(str,a,b-a-2)
	...
	executeglobal(tmp)  '在全局空间中执行
	htm = replace(htm,"{::vbcrlf}",vbcrlf)
	htm = replace(htm,"{::vbcr}",vbcr)
	ob_get_contents = htm : htm = htm_t
end function

'过程：动态包含文件
sub include(path)
	echo file_execute(path)
end sub
...
'全启变量：服务器、客户端IP地址
dim ip_server : ip_server = Request.ServerVariables("LOCAL_ADDR")
dim ip_remote : ip_remote = Request.ServerVariables("REMOTE_ADDR")
		...
		if s = "&" then
			for j=1 to 10
				...
				elseif s = " " then
					...
					tmp = tmp & "&"
					exit for
				elseif s=";" and j>1 then
					tmp = tmp & mid(str,i,j+1)
					n = n + 1
					i = i + j
					exit for
				end if
...
dim str_month_en : str_month_en = array("","January","February","March","April","May","June","July","August","September","October","November","December")
dim str_weeks_en : str_weeks_en = array("","Monday","Tuesday","Wednesday","Thursday","Friday","Saturday","Sunday")
dim str_month_cn : str_month_cn = array("","一","二","三","四","五","六","七","八","九","十","十一","十二")
dim str_weeks_cn : str_weeks_cn = array("","一","二","三","四","五","六","日")

'函数：返回格式化的时间字串
function str_time(format,byval str)
	if trim(str)="" or not isdate(str) then exit function
	dim tmp : tmp = format
	tmp = replace(tmp,"yy",right("0"&year(str),2),1,-1,0)
	tmp = replace(tmp,"y",year(str),1,-1,0)
	tmp = replace(tmp,"mm",right("0"&month(str),2),1,-1,0)
	tmp = replace(tmp,"m",month(str),1,-1,0)
	tmp = replace(tmp,"dd",right("0"&day(str),2),1,-1,0)
	tmp = replace(tmp,"d",day(str),1,-1,0)
	tmp = replace(tmp,"hh",right("0"&hour(str),2),1,-1,0)
	tmp = replace(tmp,"h",hour(str),1,-1,0)
	tmp = replace(tmp,"ii",right("0"&minute(str),2),1,-1,0)
	tmp = replace(tmp,"i",minute(str),1,-1,0)
	tmp = replace(tmp,"ss",right("0"&second(str),2),1,-1,0)
	tmp = replace(tmp,"s",second(str),1,-1,0)
	tmp = replace(tmp,"#MM",left(str_month_en(month(str)),3),1,-1,0)
	tmp = replace(tmp,"#M",str_month_cn(month(str)),1,-1,0)
	tmp = replace(tmp,"#WW",left(str_weeks_en(weekday(str)),3),1,-1,0)
	tmp = replace(tmp,"#W",str_weeks_cn(weekday(str)),1,-1,0)
	str_time = tmp
end function
...
'	next
'	str_query = tmp
	tmp = str_replace(del&"(=[^&]*){0,1}[&]{0,1}",tmp&"&","")
	str_query = str_replace("&$",tmp,"")
end function
...
'函数：用空格补齐字符串
function str_space(str, num, ext)
	if len(str) >= num then
		str_space = str & ext
	else
		str_space = str & space(num - len(str)) & ext
	end if
end function
...
function file_execute(path)
	file_execute = ob_get_contents(file_iread(path))
end function
...
function xml_load(xml)
	dim dom
	set dom = Server.CreateObject(OBJ_DOM)
	if dom.loadxml(xml) then
		set xml_load = dom
	else
		if dom.load(Server.MapPath(xml)) then
			set xml_load = dom
		else
			set xml_load = nothing
		end if
	end if
end function

'函数：返回指定名称的单个结点
function xml_node(byval dom,query)
	dim node
	set node = dom.selectSingleNode(query)
	if typename(node) = "Nothing" then
		set dom = xml_load("<null></null>")
		set xml_node = dom.documentElement
	else
		set xml_node = node
	end if
end function
...
'函数：以字符串形式返回结点属性
function xml_attribute(byval node,str)
	dim tmp
	tmp = node.getAttribute(str)
	if isnull(tmp) then tmp = "" else tmp = cstr(tmp)
	xml_attribute = tmp
end function

'函数：以字符串形式返回指定名称结点属性
function xml_iattribute(byval dom,query,str)
	dim tmp, node
	set node = dom.selectSingleNode(query)
	tmp = node.getAttribute(str)
	if isnull(tmp) then tmp = "" else tmp = cstr(tmp)
...
'翻页类 For EasyIDE v2.2
class pager_class
	private m_page   '当前页
	private m_size   '页大小
	private m_count  '总页数
	private m_total  '总记录数
	private m_link   '翻页链接
	private m_length '翻页条半径
	private m_turner '翻页条
	
	private m_rst   '当前页记录集
	...
	public property get turner
		turner = m_turner
	end property
	
	'初始化翻页链接
	private sub initlink
		dim tmp : tmp = str_query("page")
		if trim(tmp) = "" then
			m_link = "?page={$p}"
		else
			m_link = "?"&tmp&"&page={$p}"
	...
	private sub class_initialize
		m_page   = Request.QueryString("page")
		m_size   = 10
		m_count  = 0
		m_total  = 0
		initlink()
	...
	private sub class_terminate
		if typename(m_rst) = "Recordset" then
			if m_rst.state = 1 then m_rst.close : set m_rst = nothing
		end if
	end sub
	
	'翻页处理函数
	public function parse(sql,fid,conn)
		'/////////////////////////预处理////////////////////////////
		'总记录数
		set m_rst = ado_iquery(sql,conn,3,1)
		m_total = m_rst.recordcount
		'总页数
		m_rst.pagesize = m_size
		m_count = m_rst.pagecount
		'翻页参数
		if m_page = "last" then m_page = m_count
		if trim(m_page)="" or not isnumeric(m_page) then m_page = 1
		m_page = cdbl(m_page)
		if m_page <= 0 then m_page = 1
		if m_page > m_count then m_page = m_count
		m_page = cint(m_page)
		'当前页ID列表
		dim i, s, fter : s = "" : fter = fid&"="
		if not m_rst.eof then m_rst.absolutepage = m_page
		for i = 1 to m_size
			if not m_rst.eof then
				fter = fter & s & m_rst(fid)
				s = " or "&fid&"="
				m_rst.movenext
			else
				exit for
		...
		next
		'die fter
		if m_total > 0 then m_rst.filter = fter
		'/////////////////////////翻页条////////////////////////////
		dim page_start, page_end, p
		'起始页、结束页
		page_start = m_page - m_length
		page_end   = m_page + m_length
		if page_start <= 0 then
			page_end = page_end + abs(page_start)
			page_start = 1
		end if
		if page_end > m_count then page_end = m_count
		'翻页条开始
		dim page_prev, page_next
		m_turner = "<div class=""turner"">"
		'上一页
		if m_page = 1 then
			page_prev = "<a title=""上一页"" href=""javascript:void(0)"">&#8249;&#8249;</a>"
		else
			page_prev = "<a title=""上一页"" href="""& replace(m_link,"{$p}",(m_page-1)) &""">&#8249;&#8249;</a>"
		end if
		'首页
		if page_start > 1 then m_turner = m_turner & "<a title=""首页"" href="""& replace(m_link,"{$p}","1") & """>1...</a>"
		'第_页
		for p = page_start to page_end
			if p = m_page then
				m_turner = m_turner & "<a title=""第"& p &"页"" class=""c"">"& p &"</a>"
			else
				m_turner = m_turner & "<a title=""第"& p &"页"" href="""& replace(m_link,"{$p}",p) &""">"& p &"</a>"
			end if
		next
		'下一页
		if m_page = m_count then
			page_next = "<a title=""下一页"" href=""javascript:void(0)"">&#8250;&#8250;</a>"
		else
			page_next = "<a title=""下一页"" href="""& replace(m_link,"{$p}",(m_page+1)) &""">&#8250;&#8250;</a>"
		end if
		'末页
		if page_end < m_count then m_turner = m_turner&"<a title=""末页"" href="""&replace(m_link,"{$p}",m_count)&""">..."&m_count&"</a>"
		m_turner = m_turner & page_prev & page_next
		m_turner = m_turner & "<span>"& m_size &"条<cite>|</cite>页&nbsp;共<label id=""total"">"& m_total &"</label>条</span>"
		m_turner = m_turner & "</div>"
		if m_total = 0 then m_turner = ""
